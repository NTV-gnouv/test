...
public final class MainController extends BaseController {
    ...
    public void initialize(URL location, ResourceBundle resources) {
        ...
        if (!MailUtil.validate(getCurrentUser())) {
            AlertUtil.showError(
                    "The configuration file contains an invalid email, correct the error and restart the application.");
            System.exit(1);
        }
        ...
        emailTableView.setRowFactory(param -> new TableRow<>() {
            ...
            protected void updateItem(Email item, boolean empty) {
                ...
                // Row tooltip
                String tooltipDate = EmailTableView.simpleDateFormat
                        .format(Date.from(Instant.ofEpochSecond(item.getDateSent())));
                ...
                setOnMouseClicked(event -> {
                    ...
                    loadEmail(email.getUUID(), (response, args) -> {
                        ...
                        ReadController composeController = (ReadController) sceneSwitcher()
                                .getController(AppScene.READ);
    ...
    private ContextMenu newContextMenu(Email item) {
        ...
        miDelete.setOnAction(event -> clientMail().sendCmd(Command.DELETE_EMAIL, (response, args) -> {
            if (response != ServerResponse.OK) {
                setStatusLabel(response.toString());
                return;
            }
        ...
        miRestore.setOnAction(event -> clientMail().sendCmd(Command.RESTORE_EMAIL, (response, args) -> {
            if (response != ServerResponse.OK) {
                setStatusLabel(response.toString());
                return;
            }
